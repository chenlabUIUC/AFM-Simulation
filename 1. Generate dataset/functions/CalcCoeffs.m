function coef = CalcCoeffs(const, bias, pos, atom_pos, cutoff_range, convert)
    coef = zeros(1,21);
...
%         coef(1) = sum(temp) * qi;
    % vdW 12
        for i = 1:13
            temp = double(const.coef12(i)) .* Rj.^(i-1) .* 1e-10^(13-i)...
                   .* sqrt(Dj) .* dz ./ (r.^14) * -12;
            coef(1+i) = sum(temp);
%             coef(1+i) = sum(temp) * sqrt(Di) * Ri^(13-i);
    ...
    % vdW 6
        for i = 1:7
            temp = double(const.coef6(i)) .* Rj.^(i-1) .* 1e-10^(7-i)...
                   .* sqrt(Dj) .* dz ./ (r.^8) * -6;
            coef(14+i) = sum(temp);
