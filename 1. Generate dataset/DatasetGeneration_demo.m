...
% truth. The demo version just generate 3 images.
%
% INPUT:
...
if Params.check_generated
    figure(1); tiledlayout(n_generation, 6,'TileSpacing','tight','Padding','compact')
    set(gcf,'Position',[-1774 -32 1717 740/3*n_generation]);
end
...
function [I, cm, cM, mat_sz] = ImageGeneration(params)
    ...
    z_mat = FindZeros(params.d_list, Force_mat2, params.init);
        ...
        % % Plot random force curves
        % ftemp = reshape(Force_mat,[],length(params.d_list));
        % % select_inds = randi([1,size(ftemp,1)],[3,1]); %[22466;20761;19349]
        % select_inds = [22466;20761;19349];
        % figure(); hold on;
        % for i = select_inds
        %     plot(params.d_list, ftemp(i,:))
        % end
        % xlim([-2,8])
        % ylim([-2e-7, 8e-7])
        % xlabel('z (A)')
    ...
    imshow(flipud(I))
    clim auto; colorbar;
    title(['(',num2str(params.Tip(1),'%.2f'),', ',...
           num2str(log10(params.Tip(2)/Const.D_convert),'%.1f'),', ',...
           num2str(params.Tip(3),'%.2f'),')'])
    ...
    % Calibrate surface layer intensity
    Im = imgaussfilt(I,2);
